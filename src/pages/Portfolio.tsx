...
const catColors: Record<string, string> = {
  ...
  'Designer Flooring':        'border-green-500/50 bg-green-50',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {list.map(p => (
          <div key={p.id} className={`card border ${catColors[p.cat] ?? 'border-gray-100'} group cursor-pointer hover:border-brand/40 transition-all`}>
            {/* Placeholder image area */}
            <div className="w-full h-36 rounded-lg bg-gray-200/50 mb-4 flex items-center justify-center relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-brand-gold/10 to-transparent" />
              <p className="text-brand/40 text-xs font-medium z-10">Project Image</p>
              <button className="absolute top-2 right-2 p-1.5 bg-black/60 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity text-brand">
                <Eye size={13} />
              </button>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-brand">{p.finish}</p>
